...
ROOT = HERE / "N01-v17-final"
HTML = ROOT / "index.html"
CSS = ROOT / "magazine.css"
DIAGRAM = ROOT / "diagrams" / "N01-mapa-decision.svg"
PDF = ROOT / "output" / "N01-METSI-lectura-previa-v17-final.pdf"
BASELINE_PDF = HERE / "N01-v16-final" / "output" / "N01-METSI-lectura-previa-v16-final.pdf"
...
def page_occupancies(pdf: Path) -> list[float]:
    ...
    with tempfile.TemporaryDirectory(prefix="n01-v17-occupancy-") as folder:
...
def changed_visual_pages(baseline: Path, candidate: Path) -> list[int]:
    bundled = Path("/Users/diegocarralbal/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/poppler/bin/pdftoppm")
    command = shutil.which("pdftoppm") or str(bundled)
    with tempfile.TemporaryDirectory(prefix="n01-v17-visual-regression-") as folder:
        root = Path(folder)
...
def cover_seam_metric(pdf: Path) -> dict[str, object]:
    bundled = Path("/Users/diegocarralbal/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/poppler/bin/pdftoppm")
    command = shutil.which("pdftoppm") or str(bundled)
    with tempfile.TemporaryDirectory(prefix="n01-v17-cover-seam-") as folder:
...
def cover_contrast_ratio(pdf: Path) -> dict[str, float]:
    ...
    bundled = Path("/Users/diegocarralbal/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/poppler/bin/pdftoppm")
    command = shutil.which("pdftoppm") or str(bundled)
    with tempfile.TemporaryDirectory(prefix="n01-v16-cover-contrast-") as folder:
        output = Path(folder) / "cover"
        subprocess.run(
...
def cover_change_scope(reference: Path, candidate: Path) -> dict[str, object]:
    bundled = Path("/Users/diegocarralbal/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/poppler/bin/pdftoppm")
    command = shutil.which("pdftoppm") or str(bundled)
    with tempfile.TemporaryDirectory(prefix="n01-v17-cover-scope-") as folder:
        root = Path(folder)
        for label, pdf in (("reference", reference), ("candidate", candidate)):
            subprocess.run(
...
def main() -> None:
    ...
    occupancy_changes = [
        {"page": index + 1, "v16": old, "v17": new, "delta": round(new - old, 3)}
    ...
    checks = [
        check("v8_through_v16_baselines_frozen", baseline_frozen, frozen_paths),
        ...
        check("cover_eyebrow_contrast_at_least_4_5", candidate_contrast["white_contrast_ratio"] >= 4.5, {"v16": baseline_contrast, "v17": candidate_contrast}),
        ...
        check("cover_internal_seam_removed", seam["maximum_adjacent_step"] <= 3 and max(seam["legacy_seam_samples"].values()) <= 3 and baseline_seam["maximum_adjacent_step"] > 3, {"v16": baseline_seam, "v17": seam}),
        ...
        check("expected_repagination_preserves_source", bool(visual_changes) and source_unchanged, {"changed_visual_pages": visual_changes, "source_sha256": sha256(SOURCE)}),
        check("pdf_word_inventory_identical_to_v16", Counter(candidate_tokens) == Counter(baseline_tokens), {"v16_words": len(baseline_tokens), "v17_words": len(candidate_tokens), "note": "Order shifts only because the corrected paragraph crosses the physical page boundary in v16 but not in v17; the source is byte-identical."}),
