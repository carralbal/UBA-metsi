...
def export(number: int) -> None:
    code = f"N{number:02d}"
    default_root = N00_ROOT if number == 0 else ROOT / ("N01-v18-final" if number == 1 else "N02-v14-final" if number == 2 else "N03-v9-final" if number == 3 else "N04-v9-final" if number == 4 else "N05-v9-final" if number == 5 else "N06-v9-final" if number == 6 else "N07-v9-final" if number == 7 else "N08-v9-final" if number == 8 else "N09-v9-final" if number == 9 else "N10-v9-final" if number == 10 else f"{code}-v9-editorial" if 11 <= number <= 36 else code)
    document_root = Path(os.environ.get(f"METSI_PACKAGE_ROOT_{code}", default_root)).resolve()
    source = (document_root / "index.html").resolve()
    version = os.environ.get(f"METSI_PDF_VERSION_{code}")
    output_name = f"{code}-METSI-lectura-previa-{version}.pdf" if version else ("N01-METSI-lectura-previa-v18.pdf" if number == 1 else "N02-METSI-lectura-previa-v14.pdf" if number == 2 else "N03-METSI-lectura-previa-v9.pdf" if number == 3 else "N04-METSI-lectura-previa-v9.pdf" if number == 4 else "N05-METSI-lectura-previa-v9.pdf" if number == 5 else "N06-METSI-lectura-previa-v9.pdf" if number == 6 else "N07-METSI-lectura-previa-v9.pdf" if number == 7 else "N08-METSI-lectura-previa-v9.pdf" if number == 8 else "N09-METSI-lectura-previa-v9.pdf" if number == 9 else "N10-METSI-lectura-previa-v9.pdf" if number == 10 else f"{code}-METSI-lectura-previa-v9.pdf" if 11 <= number <= 36 else os.environ.get("METSI_N00_RAW_NAME", f"{code}-METSI-lectura-previa.pdf"))
    ...
    with tempfile.TemporaryDirectory(prefix=f"metsi-{code}-chrome-", dir="/private/tmp") as profile:
        ...
        temporary_output = Path("/private/tmp") / (
            f"{code}-METSI-{'v18' if number == 1 else 'v14' if number == 2 else 'v9' if number in {3, 4, 5, 6, 7, 8, 9, 10} else 'export'}"
