#!/usr/bin/env python3
"""Controles deterministas para el manuscrito canónico de N07.
...
def main() -> int:
    ...
    anchors = {
        ...
        "Malterud et al. (2016)": r"Malterud, Siersma y Guassora \(2016\)",
        ...
        "Reglamento (UE) 2024/1689": r"Reglamento europeo 2024/1689",
    }
    anchor_results = {
    ...
    second_person_hits = re.findall(
        r"\b(?:vos|tú|usted|ustedes|podés|deberías|hacé|mirá|situá|seleccioná|usarías|convertirías|querés|viste|recordás|podrías)\b",
        quoted_removed,
        flags=re.IGNORECASE,
    )
    first_person_plural_hits = re.findall(
        r"\b(?:nos|nuestro|nuestra|nuestros|nuestras|podemos|pensemos|aprendimos|veamos|recordemos|hagamos|usamos)\b",
        quoted_removed,
        flags=re.IGNORECASE,
    )
    deferred_terms = re.findall(r"\boutcomes?\b", before_references, flags=re.IGNORECASE)
    english_residue = re.findall(
        r"\b(?:dashboard|chatbot|logs?|member checking|focus group|trade-off|insights?|workaround|prompt|offline|voz-a-texto)\b",
        before_references,
        flags=re.IGNORECASE,
    )
    ...
    results = [
        ...
        check("total_word_count", 8500 <= len(words(text)) <= 10500, len(words(text))),
        check("hotel_horizonte_conductor", before_references.count("HH-07") >= 8 and before_references.count("Hotel Horizonte") >= 4, {"HH-07": before_references.count("HH-07"), "Hotel Horizonte": before_references.count("Hotel Horizonte")}),
        check("three_hh07_applications", len(re.findall(r"^### (?:Primera|Segunda|Tercera) aplicación de HH-07", before_references, flags=re.MULTILINE)) == 3, 3),
        check("n06_input_declared", "N06 cerró con una estrategia situada" in before_references and "misión de evidencia concreta" in before_references, "declared"),
        check("n08_handoff_declared", "De N07 a N08: de lo dicho a lo realizado" in heading_names and "N07 no enseña todavía a observar" in before_references, "declared"),
        check("inference_chain_fields", all(field in before_references for field in chain_fields), chain_fields),
        check("interview_protocol_fields", all(field in before_references for field in protocol_fields), protocol_fields),
        check("reference_count", len(reference_lines) == len(anchors) == 11, len(reference_lines)),
        ...
        check("preparation_instruction", "dos de las seis preguntas" in questions_block and "protocolo breve para HH-07" in questions_block, "present"),
        ...
        check("rioplatense_interview_prompts", "Llevame" in before_references and "Contame" in before_references and "recordás" in before_references, "present only inside example prompts"),
    ...
    manifest = {
        "document": "N07",
        "stage": "content-final",
        "title": "Entrevistar no es pedir requisitos",
        "language": "es-AR",
