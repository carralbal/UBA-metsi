...
El canal externo envía al Hotel Horizonte un mensaje válido: la reserva quedó confirmada. El sistema de gestión hotelera (PMS) acepta el esquema, registra el identificador y responde con éxito. Minutos después, Recepción descubre que la habitación accesible fue reasignada porque ambos sistemas interpretaban confirmada de manera distinta.
...
Dos sistemas no están integrados porque intercambien mensajes válidos, sino cuando comparten o traducen de manera explícita significado, vigencia, efecto y manejo de la indeterminación. Un contrato que omite cualquiera de esas capas puede pasar pruebas sintácticas y producir decisiones incompatibles ante demora, duplicación o falla parcial; por eso debe probarse sobre la consecuencia operacional y conservar quién puede detener y reparar.
...
**Werner Vogels.** Expone los compromisos prácticos de consistencia, disponibilidad y evolución en servicios distribuidos.

**Jim Gray.** Sistematiza transacciones, recuperación y garantías que permiten distinguir un mensaje recibido de una operación concluida.
