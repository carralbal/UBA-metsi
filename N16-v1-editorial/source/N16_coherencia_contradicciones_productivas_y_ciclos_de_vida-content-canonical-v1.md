...
¿Cómo decidir si dos modelos se contradicen por error, por diferencia legítima de perspectiva o porque el sistema todavía no resolvió una tensión real?
...
HH-15 dejó cinco modelos para decidir la validación previa. El proceso define habitación entregable cuando inspección, asignación y acceso están listos. La máquina de estados permite “entregable” antes de asignar porque fue diseñada para inventario. El journey utiliza la palabra cuando el huésped recibe confirmación. El tablero la cuenta cuando Housekeeping termina.

Cada uso nació de una pregunta distinta. La misma etiqueta crea una falsa coherencia. Dirección cree observar promesas cumplidas; Recepción, capacidad de asignar; Housekeeping, tarea finalizada. Un indicador baja mientras la espera del huésped sube.

HH-16 no impondrá una definición única por decreto. Construirá correspondencias, alcance y condiciones. Puede conservar términos distintos, como “preparada”, “asignable”, “accesible” y “confirmada”, y definir cuándo juntos sostienen “entregable para R73”. La contradicción semántica impulsa una mejora del sistema.

## Tesis

La coherencia entre modelos no exige que contengan los mismos elementos ni que coincidan literalmente. Exige que sus afirmaciones sean compatibles con propósito, alcance, nivel, tiempo, evidencia y decisión. Dos vistas pueden diferir legítimamente y seguir formando una cartera íntegra.
...
ISO/IEC/IEEE 42010:2022 permite comprender una descripción arquitectónica como conjunto de vistas gobernadas por viewpoints y concerns. La coherencia se evalúa dentro y entre vistas según sus reglas, no por repetición de contenido.
...
También puede mezclarse nivel. Un proceso habla de capacidad institucional y arquitectura de endpoint. Las relaciones no pueden compararse directamente. Se necesita una vista puente o una correspondencia.
...
El análisis reproduce cálculos, identifica procedencia y decide qué indicador corresponde a la pregunta. También conserva la divergencia si revela tiempos del sistema.

La coherencia cuantitativa no significa una cifra única, sino comparabilidad explicada.
...
En Hotel Horizonte, Recepción puede reasignar una habitación y no puede prometer una categoría superior sin autorización comercial. El PMS permite ambas operaciones a la misma credencial. El proceso omite la segunda aprobación porque históricamente se resolvía por teléfono. Las vistas no sólo discrepan: revelan un control apoyado en memoria informal.
...
El equipo toma una reserva y recorre proceso, estados, journey, arquitectura y tablero. Registra qué afirma cada aparición de “entregable”, fuente, audiencia y tiempo.

Encuentra cuatro conceptos. Los renombra y define una regla compuesta para la promesa al huésped. El tablero deja de contar limpieza como entrega. La máquina de inventario conserva “asignable”. El journey utiliza “ingreso confirmado”.

La corrección reduce una contradicción terminológica y descubre una operacional: la cerradura puede perder vigencia después de confirmar. HH-13 ya ofrece estado ambiguo y reconciliación. N16 vincula, no redefine.
...
El contrato incluye significado, dirección, condición y responsable. Si cambia estado, se identifican procesos y métricas afectadas.

Las correspondencias pueden gestionarse como datos y generar vistas. La automatización ayuda a detectar nombres sin vínculo, pero una persona valida semántica.
...
Prioridad depende de daño y uso. Dos colores diferentes son menores salvo que codifiquen estados. Una autoridad incompatible con implementación es crítica.

El registro no debe convertirse en inventario eterno. Cada entrada tiene condición de cierre y prueba.
...
Se distingue impacto directo, indirecto y desconocido. Lo desconocido conduce a investigación, no a la afirmación “sin impacto”.

La prueba utiliza escenarios críticos y actores afectados. Una lista de archivos no basta.
...
The Open Group, mediante TOGAF, organiza el trabajo arquitectónico alrededor de concerns, stakeholders, vistas y gobierno del cambio. Ese aporte resulta útil si se evita convertirlo en inventario total. La cartera de N16 toma la preocupación concreta y pregunta qué representaciones son suficientes para gobernarla.
...
Una versión registra cambio, motivo, evidencia y decisiones afectadas. La baseline puede conservar vistas con ritmos diferentes si declara compatibilidad.

El retiro evita que un modelo histórico circule como vigente. Se conserva para trazabilidad con marca clara.
...
Una herramienta puede comparar textos y diagramas, detectar nombres diferentes y proponer enlaces. También puede homogeneizar lenguaje y borrar tensiones legítimas.

La generación automática tiende a completar ausencias. Una flecha plausible puede no tener evidencia. Toda corrección propuesta conserva fuente, confianza y revisión.
...
#### Segunda aplicación de HH-16: una regla nueva y cinco vistas afectadas

Hotel Horizonte decide que toda habitación accesible necesita verificación de cerradura antes de confirmación. El cambio afecta estado, proceso, arquitectura, journey y tablero.

La matriz de correspondencias identifica vínculos. Se actualiza regla y prueba, se modifica handoff, se agrega señal técnica, se cambia comunicación y se separa métrica.

Antes de desplegar, el equipo recorre un caso ordinario y otro de excepción. Descubre que fuera de horario no existe autoridad suplente. La contradicción evita lanzar una promesa incompleta.

El cambio se divide: primero capacidad de suplencia y observabilidad, luego regla automática. La cartera gobierna secuencia.
...
5. **Tipo de contradicción:** término, frontera, nivel, tiempo, norma, operación o medida.
...
Cada salto entre vistas utiliza correspondencia explícita. Si depende de explicación oral, se registra ausencia.

La prueba termina cuando la audiencia puede reconstruir qué se sabe, qué se espera y qué sigue abierto.
...
La prueba incluye retiro. Una representación reemplazada debe dejar de aparecer en búsquedas operativas y conservar historial.

El costo de cambio es una métrica de gobierno, no un motivo para eliminar toda documentación.
...
La prueba concluye con una persona que no participó en el cambio. Debe poder elegir la versión correcta para una decisión actual y reconstruir una pasada sin ayuda oral. Si no puede, el ciclo de vida está documentado pero no gobernado.
...
Las entradas cerradas conservan decisión y prueba. Si reaparece el patrón, se revisa mecanismo, no sólo caso.

Una revisión periódica mantiene tensión productiva sin normalizar deuda.
...
El hotel acuerda cuatro términos: preparada, asignable, accesible y confirmada. “Entregable” se reserva para la combinación relativa a una reserva y momento.

Cada vista conserva su función. El proceso muestra condiciones, el estado las valida, la arquitectura localiza fuentes, el journey comunica y el tablero mide outcomes. Las correspondencias permiten rastrear.

La prueba recorre R73 con cerradura tardía y excepción de accesibilidad. El sistema comunica pendiente, evita confirmar y escala a autoridad. Después de reparar, conserva tiempos y costo.

HH-16 cierra la contradicción semántica y deja abierta una decisión estratégica: cuánto invertir para anticipar fallas. Esa pregunta pertenece a N17.
...
La consistencia técnica produjo silencio. Las vistas derivadas comparten la misma omisión. Ninguna contradice porque ninguna incorpora experiencia independiente.

La organización agrega fuentes diversas y permite hipótesis. Aparecen desacuerdos que antes estaban ocultos. La cartera parece menos ordenada y se vuelve más verdadera.
...
Una contradicción puede ser terminológica, temporal, normativa, operacional, cuantitativa o de frontera. Clasificar permite corregir, documentar, investigar, planificar, escalar o aceptar.
...
2. ¿Qué diferencia existe entre contradicción temporal y error?
...
**Peter Checkland.** Utilizó modelos conceptuales para aprender mediante comparación y debate sobre situaciones.

**Chris Argyris y Donald Schön.** Distinguieron aprendizaje que corrige acciones de aprendizaje que revisa reglas y supuestos.

**John Sterman.** Mostró que toda representación causal depende de frontera, estructura y supuestos revisables.

**Michael Jackson.** Separó dominios del problema y máquina para evitar que la solución defina prematuramente la realidad.

**Barry Boehm.** Organizó desarrollo y compromiso alrededor de riesgo y ciclos iterativos.

**Winston Royce.** Expuso límites de secuencias lineales y necesidad de retroalimentación en desarrollo.

## Referencias base

- Checkland, P. y Poulter, J. (2006). *Learning for Action*. Wiley.
...
- ISO/IEC/IEEE (2024). *ISO/IEC/IEEE 24748-2:2024 Life Cycle Management, Guidelines for the Application of ISO/IEC/IEEE 15288*. https://www.iso.org/standard/84661.html
