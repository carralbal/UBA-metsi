...
ROOT = HERE / "N01-v15-final"
HTML = ROOT / "index.html"
CSS = ROOT / "magazine.css"
DIAGRAM = ROOT / "diagrams" / "N01-mapa-decision.svg"
PDF = ROOT / "output" / "N01-METSI-lectura-previa-v15-final.pdf"
BASELINE_PDF = HERE / "N01-v14-final" / "output" / "N01-METSI-lectura-previa-v14-final.pdf"
...
def page_occupancies(pdf: Path) -> list[float]:
    ...
    with tempfile.TemporaryDirectory(prefix="n01-v15-occupancy-") as folder:
...
def changed_visual_pages(baseline: Path, candidate: Path) -> list[int]:
    bundled = Path("/Users/diegocarralbal/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/poppler/bin/pdftoppm")
    command = shutil.which("pdftoppm") or str(bundled)
    with tempfile.TemporaryDirectory(prefix="n01-v15-visual-regression-") as folder:
        root = Path(folder)
...
def cover_seam_metric(pdf: Path) -> dict[str, float | int]:
    bundled = Path("/Users/diegocarralbal/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/poppler/bin/pdftoppm")
    command = shutil.which("pdftoppm") or str(bundled)
    with tempfile.TemporaryDirectory(prefix="n01-v15-cover-seam-") as folder:
...
def main() -> None:
    ...
    cover_bottom = cover_bottom_scrim_regression(BASELINE_PDF, PDF)
    ...
    checks = [
        check("v8_through_v14_baselines_frozen", baseline_frozen, frozen_paths),
        ...
        check("cover_overlay_pattern_extended_to_bottom_trim", any(abs(abs(item["matrix"][0]) - 1.1055) < .002 and abs(abs(item["matrix"][3]) - 1.0) < .002 and abs(item["bbox"][3] - 842.00012) < .01 and abs(item["ystep"] - 844.00012) < .01 and item["image_heights"].get("/X6") == 842 and item["image_heights"].get("/X9") == 842 for item in cover_pattern_matrices), cover_pattern_matrices),
        check("cover_bottom_scrim_has_no_horizontal_cut", cover_bottom["baseline_transition_step"] > 20 and cover_bottom["candidate_transition_step"] < 10 and cover_bottom["upper_field_max_mean_difference"] < .02 and cover_bottom["full_difference_bbox"] is not None, cover_bottom),
