...
El promedio no era falso. Respondía una pregunta demasiado pobre. La muestra reproducía la frecuencia histórica, pero contenía pocos casos severos, mensajes escritos con errores y situaciones donde una persona describía síntomas sin usar vocabulario clínico. Además, la etiqueta histórica no constituía una verdad independiente: reflejaba decisiones previas, capacidad disponible y desigualdades de acceso.

La primera explicación atribuye las fallas a falta de datos. Otra señala que la unidad de evaluación está mal definida: se puntúa un mensaje aislado aunque la tarea real incluye antecedentes, interacción posterior y decisión del equipo. Una tercera cuestiona el umbral común para todas las poblaciones. Cada hipótesis requiere evidencia distinta y puede modificar tanto el sistema como el proceso de supervisión.
...
La supervisión se somete a prueba como parte del sistema. Durante una franja de alta demanda, las personas revisan tantas salidas que comienzan a confirmar por rutina. Un control formal existe, pero no es significativo si falta tiempo, información o autoridad para contradecir. La solución reduce autonomía, eleva ciertos casos por reglas explícitas y reserva al modelo una función de apoyo. Cuando la confianza es baja o la consecuencia alta, la ruta humana recibe prioridad y contexto.
...
La evaluación de un sistema de inteligencia artificial debe medir una tarea completa y el sistema sociotécnico que la realiza, no sólo una salida del modelo. Unidad, población, línea de base y validez determinan qué representa cada resultado. Cobertura, severidad, desempeño desagregado, robustez y carga real de supervisión muestran qué autonomía puede justificarse. Si una media favorable oculta fallas concentradas, daño difícil de reparar o un control humano impracticable, la evidencia no autoriza ampliar exposición. La aprobación es siempre condicional a un uso, una configuración y una capacidad de intervención que también deben evaluarse en operación.
