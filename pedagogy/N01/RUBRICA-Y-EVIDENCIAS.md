# N01 · Rúbrica y evidencias

## Escala común

Cada criterio se observa en cuatro niveles:

- `0`: ausente o contradictorio;
- `1`: presente como afirmación, sin relación defendible;
- `2`: relacionado con evidencia y decisión, aunque conserva ambigüedades;
- `3`: preciso, trazable, revisable y sensible a límites u objeciones.

## Rúbrica de HH-01

| Criterio | Pregunta de evaluación | Evidencia de nivel 3 |
|---|---|---|
| Separación conceptual | ¿Distingue pedido, propósito, problema posible y solución preferida? | Ningún elemento ocupa dos funciones sin justificación. |
| Incertidumbre | ¿La incertidumbre está vinculada con una decisión? | Declara qué todavía no puede decidirse y por qué. |
| Procedencia | ¿Puede reconstruirse de dónde proviene cada afirmación? | Distingue pedido, observación, relato, inferencia y supuesto. |
| Proporcionalidad | ¿La próxima acción corresponde al riesgo y a la evidencia disponible? | Autoriza aprendizaje real sin comprometer una solución total. |
| Autoridad | ¿Está claro quién puede decidir, revisar, detener y reparar? | La distribución de autoridad forma parte del memo. |
| Revisabilidad | ¿Existe evidencia adversa capaz de cambiar el camino? | La condición es concreta, observable y relevante para la decisión. |
| Respuesta a objeciones | ¿La segunda versión utiliza la crítica recibida? | Mantiene o cambia con razones explícitas, no por obediencia. |

## Uso formativo

La primera versión no recibe nota numérica. La rúbrica orienta la revisión. La segunda versión puede registrarse como evidencia del portfolio, acompañada por un comentario breve del docente sobre la relación más sólida y la fisura más importante.

## Control de aprendizaje individual

El registro final debe permitir comprobar:

- qué afirmación inicial se conservó;
- qué evidencia u objeción produjo un cambio;
- qué decisión puede defenderse ahora;
- qué condición mantiene abierta la revisión;
- qué pregunta se traslada a N02.

Una respuesta que sólo enumera conceptos de N01 no demuestra el resultado del encuentro.
