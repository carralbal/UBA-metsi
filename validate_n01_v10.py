...
SOURCE = HERE / "N01" / "source" / "N01_metodologia_sin_recetas-v10.md"
CURATION = HERE / "N01" / "image-curation" / "image-manifest.json"
ROOT = HERE / "N01-v10-final"
HTML = ROOT / "index.html"
CSS = ROOT / "magazine.css"
DIAGRAM = ROOT / "diagrams" / "N01-mapa-decision.svg"
PDF = ROOT / "output" / "N01-METSI-lectura-previa-v10-final.pdf"
...
def main() -> None:
    ...
    checks = [
        check("v8_and_v9_baselines_frozen", baseline_frozen, frozen_paths),
        ...
        check("cover_meta_copy_layer", "lecturaprevia" in compact(page_texts[0]) and "edición2026" in compact(page_texts[0]), "LECTURA PREVIA / EDICIÓN 2026"),
        ...
        check("voice_caption_updated", "Seis relatos verdaderos que ninguno explica solo." in html and "La misma contradicción cambia" not in html, "new systems caption"),
        ...
        check("glossary_four_columns_complete_and_legible", "column-count:4" in css and "font-size:9.15pt" in css and all(compact(term) in compact(glossary_page) for term in glossary_terms) and compact("Preguntas de preparación") not in compact(glossary_page), {"physical_page": glossary_page_index + 1, "terms": len(glossary_terms)}),
        check("assets_preserved", len(assets) == 8 and assets_valid, {"assets": len(assets), "hashes_valid": assets_valid}),
        check("pdf_a4_29_or_30_pages", len(reader.pages) in {29, 30} and all(abs(float(page.mediabox.width) - 595.276) < 2 and abs(float(page.mediabox.height) - 841.89) < 2 for page in reader.pages), {"pages": len(reader.pages)}),
