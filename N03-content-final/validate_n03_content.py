#!/usr/bin/env python3
"""Controles deterministas para el manuscrito canónico de N03.
...
def main() -> int:
    ...
    anchors = {
        "Churchman (1968)": r"\bChurchman\b",
        "Midgley (2000)": r"\bMidgley\b",
        "Meadows (2008)": r"\bMeadows\b",
        "Senge (2006)": r"\bSenge\b",
        "Sterman (2000)": r"\bSterman\b",
        "Leveson (2012)": r"\bLeveson\b",
        "NIST AI 600-1 (2024)": r"NIST AI 600-1",
        "NIST AI 700-2 (2025)": r"\bARIA\b",
        "Reglamento UE 2024/1689": r"Reglamento europeo 2024/1689",
        "Polojärvi (2023)": r"\bPolojärvi\b",
    ...
    second_person_hits = re.findall(
        r"\b(?:vos|tú|usted|podés|deberías|hacé|mirá|situá|seleccioná)\b",
        before_references,
        flags=re.IGNORECASE,
    )
    ...
    results = [
        ...
        check("total_word_count", 7600 <= len(words(text)) <= 9800, len(words(text))),
        check("hotel_horizonte_conductor", before_references.count("HH-03") >= 5 and before_references.count("Hotel Horizonte") >= 4, {"HH-03": before_references.count("HH-03"), "Hotel Horizonte": before_references.count("Hotel Horizonte")}),
        ...
        check("no_prose_dashes", not prose_emdash_lines, prose_emdash_lines),
    ...
    manifest = {
        "document": "N03",
        "stage": "content-final",
        "title": "Fronteras, retroalimentación y efectos no intencionales",
        "language": "es-AR",
