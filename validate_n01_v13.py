...
ROOT = HERE / "N01-v13-final"
HTML = ROOT / "index.html"
CSS = ROOT / "magazine.css"
DIAGRAM = ROOT / "diagrams" / "N01-mapa-decision.svg"
PDF = ROOT / "output" / "N01-METSI-lectura-previa-v13-final.pdf"
BASELINE_PDF = HERE / "N01-v12-final" / "output" / "N01-METSI-lectura-previa-v12-final.pdf"
...
def page_occupancies(pdf: Path) -> list[float]:
    ...
    with tempfile.TemporaryDirectory(prefix="n01-v13-occupancy-") as folder:
...
def changed_visual_pages(baseline: Path, candidate: Path) -> list[int]:
    bundled = Path("/Users/diegocarralbal/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/poppler/bin/pdftoppm")
    command = shutil.which("pdftoppm") or str(bundled)
    with tempfile.TemporaryDirectory(prefix="n01-v13-visual-regression-") as folder:
        root = Path(folder)
...
def cover_seam_metric(pdf: Path) -> dict[str, float | int]:
    bundled = Path("/Users/diegocarralbal/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/poppler/bin/pdftoppm")
    command = shutil.which("pdftoppm") or str(bundled)
    with tempfile.TemporaryDirectory(prefix="n01-v13-cover-seam-") as folder:
...
def main() -> None:
    ...
    checks = [
        check("v8_through_v12_baselines_frozen", baseline_frozen, frozen_paths),
        ...
        check("cover_overlay_pattern_scaled_to_trim", any(abs(abs(item["matrix"][0]) - 1.1055) < .002 and abs(abs(item["matrix"][3]) - 1.1055) < .002 for item in cover_pattern_matrices), cover_pattern_matrices),
