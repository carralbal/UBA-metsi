...
ROOT = HERE / "N02-v13-final"
BASE_ROOT = HERE / "N02-v12-final"
BASE_SOURCE = BASE_ROOT / "source" / "N02_el_sistema_no_cabe_en_una_aplicacion-v12.md"
BASE_PDF = BASE_ROOT / "output" / "N02-METSI-lectura-previa-v12-final.pdf"
SOURCE = ROOT / "source" / "N02_el_sistema_no_cabe_en_una_aplicacion-v13.md"
PDF = ROOT / "output" / "N02-METSI-lectura-previa-v13-final.pdf"
...
def page_occupancies(pdf: Path) -> list[float]:
    ...
    with tempfile.TemporaryDirectory(prefix="n02-v13-occupancy-") as folder:
...
def changed_visual_pages(reference: Path, candidate: Path) -> list[int]:
    bundled = Path("/Users/diegocarralbal/.cache/codex-runtimes/codex-primary-runtime/dependencies/native/poppler/poppler/bin/pdftoppm")
    command = shutil.which("pdftoppm") or str(bundled)
    with tempfile.TemporaryDirectory(prefix="n02-v13-regression-") as folder:
        root = Path(folder)
...
def text_extents(pdf: Path) -> dict[int, dict[str, float]]:
    result: dict[int, dict[str, float]] = {}
    with pdfplumber.open(pdf) as document:
        for page_number, page in enumerate(document.pages, 1):
            lines = [line for line in page.extract_text_lines() if 40 <= float(line["top"]) < 790]
            objects = [(float(line["top"]), float(line["bottom"])) for line in lines]
...
def question_panel_tail(pdf: Path) -> dict[str, float]:
    with pdfplumber.open(pdf) as document:
        page = document.pages[24]
        lines = [line for line in page.extract_text_lines() if 40 <= float(line["top"]) < 790]
        last_content = max(float(line["bottom"]) for line in lines)
        panels = [
            rect for rect in page.rects
            if float(rect["width"]) > 400 and 150 < float(rect["height"]) < 600
        ]
        panel_bottom = max(float(rect["bottom"]) for rect in panels)
        return {
            "last_content_y": round(last_content, 2),
            "panel_bottom_y": round(panel_bottom, 2),
            "tail_points": round(panel_bottom - last_content, 2),
        }
...
def main() -> None:
    ...
    text_diff_exact = BASE_SOURCE.read_bytes() == SOURCE.read_bytes()
    ...
    extents = text_extents(PDF)
    panel_tail = question_panel_tail(PDF)
    ...
    checks = [
        ...
        check("v12_to_v13_source_is_byte_identical", text_diff_exact and Counter(source_tokens) == Counter(base_tokens) and sha256(BASE_SOURCE) == sha256(SOURCE), {"byte_identical": text_diff_exact, "v12_sha256": sha256(BASE_SOURCE), "v13_sha256": sha256(SOURCE), "v12_tokens": len(base_tokens), "v13_tokens": len(source_tokens)}),
        ...
        check("glossary_split_by_complete_column", glossary_counts == {24: 256, 25: 131} and sum(glossary_counts.values()) == 387 and all(compact(token) in compact(page_texts[24]) for token in ("PMS (sistema de gestión hotelera)", "Propiedad emergente", "Reconciliación", "Sistema de trabajo", "Socio-técnico", "Stakeholder o parte interesada")), glossary_counts),
        check("list_blocks_do_not_split", all(token in css for token in (".pill-summary .section-body ol{break-inside:avoid-page", ".questions .section-body ol{display:block", ".n02-glossary-continuation{width:100%;columns:3")) and css.count("page-break-inside:avoid") >= 3, "pills, complete glossary entries and questions protected"),
        check("question_spacing_is_uniform_and_in_range", "grid-template-rows" not in re.search(r'\.premium-magazine\.document-n02 \.questions \.section-body ol\{([^}]*)\}', css).group(1) and "columns:2" in css and len(question_gaps) == 4 and min(question_gaps) >= 35.0 and max(question_gaps) <= 50.0 and max(question_gaps) - min(question_gaps) <= 1.0, {"gaps_points": question_gaps, "spread": round(max(question_gaps) - min(question_gaps), 2)}),
        check("three_column_legible_glossary", "font-size:9.7pt" in css and glossary_counts == {24: 256, 25: 131}, "approved 8.8 pt rendered size; six complete continuation entries in three columns"),
        check("page_24_and_25_fill_targets", extents[24]["fill"] >= .55 and extents[25]["fill"] >= .55 and extents[25]["bottom"] >= 475, {"page_24": extents[24], "page_25": extents[25]}),
        check("question_panel_tracks_content", panel_tail["tail_points"] <= 30.0, panel_tail),
        ...
        check("expected_visual_change_scope", visual_changes == [24, 25], {"changed_pages": visual_changes, "cover_unchanged": 1 not in visual_changes}),
