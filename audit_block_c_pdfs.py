#!/usr/bin/env python3
"""Deterministic integrity and publication audit for METSI N11–N16 PDFs."""
...
def tokens(text: str) -> list[str]:
    ...
    text = re.sub(r"\b([a-z]{2,})-\s+(\d+)\b", r"\1-\2", text)
    return re.findall(r"[a-záéíóúüñ0-9]+(?:-[a-záéíóúüñ0-9]+)*", text)
...
def main() -> None:
    records = [audit(number) for number in range(11, 17)]
    lines = [
        "# METSI · Auditoría de publicación del Bloque C",
        "",
        "Control determinístico posterior a la composición y finalización de N11 a N16.",
        "",
    ...
    (ROOT / "BLOCK-C-PDF-AUDIT.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
