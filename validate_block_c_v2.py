#!/usr/bin/env python3
"""Gate determinista, exhaustivo y de solo lectura para N11-N36 v3 editorial.
...
def render_pdf(pdf: Path) -> list[dict[str, Any]]:
    ...
    records: list[dict[str, Any]] = []
    with tempfile.TemporaryDirectory(prefix="metsi-block-c-v3-") as folder:
...
def audit(number: int) -> dict[str, Any]:
    ...
    package = HERE / f"{document}-v3-editorial"
    ...
    core = {
        ...
        "pdf": package / "output" / f"{document}-METSI-lectura-previa-v3-final.pdf",
        "raw_pdf": package / "output" / f"{document}-METSI-lectura-previa-v3.pdf",
    }
    missing_core = sorted(key for key, path in core.items() if not path.is_file())
    checks.append(check("required_v3_package_files_exist", not missing_core, {"missing": missing_core}))
    if missing_core:
        return finalize_report(document, package.name, checks, {})
...
def finalize_report(document: str, package: str, checks: list[dict[str, Any]], metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "document": document, "package": package, "version": "v3-editorial",
...
def main() -> int:
    ...
    for number in range(args.start, args.end + 1):
        try:
            ...
            report = {
                "document": f"N{number:02d}", "package": f"N{number:02d}-v3-editorial",
                "version": "v3-editorial", "validator": Path(__file__).name, "mode": "read-only",
