#!/usr/bin/env python3
"""Auditoría reproducible del contenido canónico de METSI N12."""
...
SOURCE = PACKAGE / "source" / "N12_eventos_estados_comandos_evidencia_y_autoridad-content-canonical-v1.md"
...
PREDECESSORS["N11"] = ROOT / "N11-content-canonical" / "source" / "N11_cuando_un_dato_sostiene_una_afirmacion-content-canonical-v1.md"
...
def split_blocks(text: str) -> list[dict[str, object]]:
    ...
    for index, block in enumerate(blocks, 1):
        block["source_id"] = f"N12-S{index:03d}"
...
def main() -> int:
    ...
    substantive = text[text.index("## La inscripción") : text.index("## Cinco píldoras para recordar")]
    references = text.split("## Referencias base", 1)[1]
    body_without_references = text.split("## Referencias base", 1)[0]
    blocks = split_blocks(text)

    current_tokens = tokens(body_without_references)
    current_ngrams = {tuple(current_tokens[i : i + 24]) for i in range(max(0, len(current_tokens) - 23))}
    ...
    for code, path in PREDECESSORS.items():
        ...
        for index in range(max(0, len(predecessor_tokens) - 23)):
            ...
            if gram in current_ngrams and gram not in seen:
    ...
    recent_references = re.findall(r"\(202[2-6]\)", references)
    urls = re.findall(r"https://[^\s)]+", references)
    instrument = text[text.index("### Instrumento HH-12") : text.index("### Prueba de coherencia")]

    prohibited_register = re.findall(r"\b(?:tú|vos|usted|seleccioná|elegí|hacé|podés|deberías)\b", text, flags=re.IGNORECASE)
    editorial_placeholders = re.findall(r"\b(?:TBD|XXX|TODO)\b|\blorem\b|\[[^\]]+\]", text)
    stray_dashes = re.findall(r"[—–]", body_without_references)
    required_terms = [
        "HH-11", "HH-12", "N11", "N13", "comando", "evento", "estado", "consulta",
        "evidencia", "autoridad", "rectificación", "retractación", "compensación",
        "correlación", "Hotel Horizonte", "inteligencia artificial",
    ]
    reference_anchors = {
        "Evans": "Eric Evans",
        "Lamport": "Leslie Lamport",
        "Fowler Event Sourcing": "Martin Fowler denomina *event sourcing*",
        "Fowler CQRS": "Martin Fowler advierte",
        "Ferraiolo and Kuhn": "David Ferraiolo y Richard Kuhn",
        "Sandhu": "Ravi Sandhu",
        "Leveson": "Nancy Leveson",
        "UML": "UML 2.5.1",
        "BPMN": "BPMN 2.0.2",
        "CloudEvents": "CloudEvents 1.0.2",
        "AsyncAPI": "AsyncAPI 3.1.0",
        "NIST AI RMF": "NIST AI RMF 1.0",
        "NIST AI 600-1": "perfil para IA generativa de 2024",
    ...
    results = [
        ...
        check("total_word_count", len(words(text)) >= 7500, len(words(text))),
        check("required_architecture", all(title in section_titles for title in (
            ...
            "Límites y tensiones", "Síntesis", "Cinco píldoras para recordar", "Glosario esencial",
            "Preguntas de preparación", "Referentes", "Referencias base",
        )), section_titles),
        ...
        check("hh12_instrument_nine_fields", len(re.findall(r"^[1-9]\. \*\*", instrument, flags=re.MULTILINE)) == 9, "nine fields"),
        ...
        check("reference_urls", len(urls) >= 10, len(urls)),
    ...
    report = {
        "document": "N12",
        "title": "Eventos, estados, comandos, evidencia y autoridad",
